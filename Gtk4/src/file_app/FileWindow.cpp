...
static void file_window_init(FileWindow *self)
{
    ...
    // Initalize window
    gtk_window_set_title(GTK_WINDOW(self), "File Browser");
    gtk_window_set_icon_name(GTK_WINDOW(self), "file-app");
    ...
    self->btn_new = gtk_button_new_from_icon_name("document-new");
...
FileWindow *file_window_new(GtkWindow *win, int width, int height)
{
    // Create a new file win and set default size
    FileWindow *file_win = FILE_WINDOW(g_object_new(file_window_get_type(),
                                                    "transient-for", win, NULL));
    gtk_window_set_default_size(GTK_WINDOW(file_win), width, height);
    return file_win;
}
